package org.example.TaskHashMap;
...
public class OrdersManager {
    ...
    String getMaxOrderCustomerName() {
        ...
        for (String name : customersOrders.keySet()) {
            List<Double> listOrders = customersOrders.get(customerName);
            double sumOrders = 0.0;
            for (Double order : listOrders) {
                sumOrders += order;// Допишите логику работы метода
            }
        ...
        void removeUnprofitableOrders() {
            ...
            for (String name : names) {
                if (customersOrders.containsKey(name)) {
                    ...
                    System.out.println("Клиента " + name + " больше нет в таблице.");// Удалите из хеш-таблицы тех, чьи расходы не превышают 5000
                }
            }
        }
}